...
static const char long_gen21[] =
    "Internal fields in an attribute have been overwritten; possible errors in using the attribute value in user code.";
...
static const char long_gen50[] =
    "Size of Cartesian grid is larger than the size of the communicator";
...
static const char long_gen57[] =
    "failure occurred while performing local cancellation of a eager message";
static const char short_gen58[] = "**ch3|cancelreq";
static const char long_gen58[] =
    "failure occurred while sending remote cancellation request packet";
...
static const char long_gen60[] =
    "failure occurred while performing local cancellation of a rendezvous message";
...
static const char long_gen67[] =
    "UNIMPLEMENTED: unable to process out-of-order cancellation requests";
...
static const char long_gen102[] =
    "failure occurred while attempting to send eager synchronization packet";
...
static const char long_gen105[] =
    "An intercommunicator is required but an intracommunicator was provided.";
static const char short_gen106[] = "**commnotintra";
static const char long_gen106[] =
    "An intracommunicator is required but an intercommunicator was provided.";
...
static const char long_gen116[] =
    "m * nprocs is less than array_size and is not valid for block(m) distribution (optional arguments: {m*nprocs}, {array_size} (int))";
...
static const char long_gen120[] =
    "The requested datarep name has already been specified to MPI_REGISTER_DATAREP";
...
static const char long_gen128[] =
    "Receiving data with a datatype whose signature does not match that of the sending datatype.";
...
static const char long_gen132[] =
    "Local and remote groups in MPI_Intercomm_create must not contain the same processes";
...
static const char long_gen198[] =
    "An address does not fit into a Fortran INTEGER.  Use MPI_Get_address instead";
...
static const char long_gen205[] =
    "Cannot use this function when the file is opened with amode MPI_MODE_SEQUENTIAL ";
...
static const char long_gen214[] =
    "disp must be set to MPI_DISPLACEMENT_CURRENT since file was opened with MPI_MODE_SEQUENTIAL";
...
static const char long_gen227[] =
    "Only one active split collective I/O operation is allowed per file handle";
...
static const char long_gen472[] =
    "failed to make progress on the shared memory bootstrap message queue";
...
static const char long_gen495[] =
    "Attempt to complete a request with MPI_GREQUEST_COMPLETE that was not started with MPI_GREQUEST_START";
...
static const char long_gen500[] =
    "Could not determine which group to place first in merged intracommunicator.  Please use the parameter high to choose which group should be placed first.";
...
static const char long_gen559[] =
    "Persistent request passed to MPI_Start or MPI_Startall is already active.";
...
static const char long_gen580[] =
    "Shared memory subsystem unable to allocate a block of the requested size";
...
static const char long_gen594[] =
    "a memory fault occurred while accessing the host description string";
...
static const char long_gen620[] =
    "MPL_malloc failed to allocate memory for an event queue structure";
...
static const char long_gen648[] =
    "The value of typeclass is not one of MPI_TYPECLASS_REAL, MPI_TYPECLASS_INTEGER, or MPI_TYPECLASS_COMPLEX";
...
static const char long_spc19[] =
    "Insufficient space in Bsend buffer; requested %d; total buffer size is %d";
...
static const char long_spc22[] =
    " Cartesian coordinate for the %d coordinate is %d but must be between 0 and %d";
static const char short_spc23[] = "**cartdim %d %d";
static const char long_spc23[] =
    "Size of the communicator (%d) is smaller than the size of the Cartesian topology (%d)";
...
static const char long_spc35[] =
    "[ch3:sock] spawned group unable to connect back to the parent on port <%s>";
...
static const char long_spc38[] =
    "[ch3:sock] rank %d unable to connect to rank %d using business card <%s>";
...
static const char long_spc42[] =
    "Connection refused for process group %d, rank %d, business card <%s>";
...
static const char long_spc45[] =
    "m * nprocs is %d but must equal the array size %d and is not valid for block(m) distribution";
static const char short_spc46[] = "**darraycyclic %d";
static const char long_spc46[] =
    "Value of m is %d but must be positive for a cyclic(m) distribution";
static const char short_spc47[] = "**darraydist %d %d";
static const char long_spc47[] =
    "For MPI_DISTRIBUTE_NONE, the value of array_of_psizes[%d] is %d but must have value 1";
...
static const char long_spc51[] =
    "Local and remote groups in MPI_Intercomm_create must not contain the same processes; both contain process %d";
static const char short_spc52[] = "**edgeoutrange %d %d %d";
static const char long_spc52[] =
    "Edge index edges[%d] is %d but must be nonnegative and less than %d";
...
static const char long_spc61[] =
    "failed to create a sysv key from the file '%s' and id %d, error %d";
static const char short_spc62[] = "**ftruncate %s %d %d";
static const char long_spc62[] =
    "unable to resize the shared memory object %s to size %d (errno %d)";
...
static const char long_spc65[] =
    "Rank %d of the specified group is not a member of this communicator";
static const char short_spc66[] = "**hostlookup %d %d %s";
static const char long_spc66[] =
    "Host lookup failed for process group %d, rank %d, business card <%s>";
...
static const char long_spc70[] =
    "Index values in graph topology must be monotone nondecreasing but index[%d] is %d but the next index value is %d";
...
static const char long_spc83[] =
    "MPI_Accumulate(origin_addr=%p, origin_count=%d, %D, target_rank=%d, target_disp=%d, target_count=%d, %D, %O, %W) failed";
...
static const char long_spc88[] =
    "MPI_Allgather(sbuf=%p, scount=%d, %D, rbuf=%p, rcount=%d, %D, %C) failed";
static const char short_spc89[] = "**mpi_allgatherv %p %d %D %p %p %p %D %C";
static const char long_spc89[] =
    "MPI_Allgatherv(sbuf=%p, scount=%d, %D, rbuf=%p, rcounts=%p, displs=%p, %D, %C) failed";
...
static const char long_spc92[] =
    "MPI_Alltoall(sbuf=%p, scount=%d, %D, rbuf=%p, rcount=%d, %D, %C) failed";
static const char short_spc93[] = "**mpi_alltoallv %p %p %p %D %p %p %p %D %C";
static const char long_spc93[] =
    "MPI_Alltoallv(sbuf=%p, scnts=%p, sdispls=%p, %D, rbuf=%p, rcnts=%p, rdispls=%p, %D, %C) failed";
static const char short_spc94[] = "**mpi_alltoallw %p %p %p %p %p %p %p %p %C";
static const char long_spc94[] =
    "MPI_Alltoallw(sbuf=%p, scnts=%p, sdispls=%p, stypes=%p, rbuf=%p, rcnts=%p, rdispls=%p, rtypes=%p, %C) failed";
...
static const char long_spc101[] =
    "MPI_Bsend_init(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
...
static const char long_spc106[] =
    "MPI_Cart_create(%C, ndims=%d, dims=%p, periods=%p, reorder=%d, comm_cart=%p) failed";
static const char short_spc107[] = "**mpi_cart_get %C %d %p %p %p";
static const char long_spc107[] =
    "MPI_Cart_get(%C, maxdims=%d, dims=%p, periods=%p, coords=%p) failed";
static const char short_spc108[] = "**mpi_cart_map %C %d %p %p %p";
static const char long_spc108[] =
    "MPI_Cart_map(%C, ndims=%d, dims=%p, periods=%p, newrank=%p) failed";
...
static const char long_spc110[] =
    "MPI_Cart_shift(%C, direction=%d, displ=%d, source=%p, dest=%p) failed";
...
static const char long_spc114[] =
    "MPI_Comm_accept(port=\"%s\", %I, root=%d, %C, newcomm=%p) failed";
...
static const char long_spc117[] =
    "MPI_Comm_connect(port=\"%s\", %I, root=%d, %C, newcomm=%p) failed";
...
static const char long_spc120[] =
    "MPI_Comm_create_keyval(comm_copy_attr_fn=%p, comm_delete_attr_fn=%p, comm_keyval=%p, extra_state=%p) failed";
...
static const char long_spc126[] =
    "MPI_Comm_get_attr(%C, comm_keyval=%d, attribute_val=%p, flag=%p) failed";
...
static const char long_spc139[] =
    "MPI_Comm_spawn(cmd=\"%s\", argv=%p, maxprocs=%d, %I, root=%d, %C, intercomm=%p, errors=%p) failed";
static const char short_spc140[] = "**mpi_comm_spawn_multiple %d %p %p %p %p %d %C %p %p";
static const char long_spc140[] =
    "MPI_Comm_spawn_multiple(count=%d, cmds=%p, argvs=%p, maxprocs=%p, infos=%p, root=%d, %C, intercomm=%p, errors=%p) failed";
...
static const char long_spc157[] =
    "MPI_Gather(sbuf=%p, scount=%d, %D, rbuf=%p, rcount=%d, %D, root=%d, %C) failed";
static const char short_spc158[] = "**mpi_gatherv %p %d %D %p %p %p %D %d %C";
static const char long_spc158[] =
    "MPI_Gatherv failed(sbuf=%p, scount=%d, %D, rbuf=%p, rcnts=%p, displs=%p, %D, root=%d, %C) failed";
static const char short_spc159[] = "**mpi_get %p %d %D %d %d %d %D %W";
static const char long_spc159[] =
    "MPI_Get(origin_addr=%p, origin_count=%d, %D, target_rank=%d, target_disp=%d, target_count=%d, %D, %W) failed";
...
static const char long_spc165[] =
    "MPI_Graph_create(%C, nnodes=%d, index=%p, edges=%p, reorder=%d, comm_graph=%p) failed";
static const char short_spc166[] = "**mpi_graph_get %C %d %d %p %p";
static const char long_spc166[] =
    "MPI_Graph_get(%C, maxindex=%d, maxedges=%d, index=%p, edges=%p) failed";
static const char short_spc167[] = "**mpi_graph_map %C %d %p %p %p";
static const char long_spc167[] =
    "MPI_Graph_map(%C, nnodes=%d, index=%p, edges=%p, newrank=%p) failed";
static const char short_spc168[] = "**mpi_graph_neighbors %C %d %d %p";
static const char long_spc168[] =
    "MPI_Graph_neighbors(%C, rank=%d, maxneighbors=%d, neighbors=%p) failed";
...
static const char long_spc172[] =
    "MPI_Grequest_start(query_fn=%p, free_fn=%p, cancel_fn=%p, extra_state=%p, request=%p) failed";
...
static const char long_spc183[] =
    "MPI_Group_translate_ranks(%G, n=%d, ranks1=%p, %G, ranks2=%p) failed";
...
static const char long_spc185[] =
    "MPI_Ibsend(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
...
static const char long_spc198[] =
    "MPI_Intercomm_create(%C, local_leader=%d, %C, remote_leader=%d, tag=%d, newintercomm=%p) failed";
...
static const char long_spc201[] =
    "MPI_Irecv(buf=%p, count=%d, %D, src=%d, tag=%d, %C, request=%p) failed";
static const char short_spc202[] = "**mpi_irsend %p %d %D %d %d %C %p";
static const char long_spc202[] =
    "MPI_Irsend(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
...
static const char long_spc204[] =
    "MPI_Isend(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
static const char short_spc205[] = "**mpi_issend %p %d %D %d %d %C %p";
static const char long_spc205[] =
    "MPI_Issend(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
static const char short_spc206[] = "**mpi_keyval_create %p %p %p %p";
static const char long_spc206[] =
    "MPI_Keyval_create(copy_fn=%p, del_fn=%p, keyval=%p, extra=%p) failed";
...
static const char long_spc212[] =
    "MPI_Pack(inbuf=%p, incount=%d, %D, outbuf=%p, outcount=%d, position=%p, %C) failed";
static const char short_spc213[] = "**mpi_pack_external %s %p %d %D %p %d %p";
static const char long_spc213[] =
    "MPI_Pack_external(datarep=%s, inbuf=%p, incount=%d, %D, outbuf=%p, outcount=%d, position=%p) failed";
static const char short_spc214[] = "**mpi_pack_external_size %s %d %D %p";
static const char long_spc214[] =
    "MPI_Pack_external_size(datarep=%s, incount=%d, %D, size=%p) failed";
...
static const char long_spc219[] =
    "MPI_Put(origin_addr=%p, origin_count=%d, %D, target_rank=%d, target_disp=%d, target_count=%d, %D, %W) failed";
...
static const char long_spc222[] =
    "MPI_Recv_init(buf=%p, count=%d, %D, src=%d, tag=%d, %C, request=%p) failed";
static const char short_spc223[] = "**mpi_reduce %p %p %d %D %O %d %C";
static const char long_spc223[] =
    "MPI_Reduce(sbuf=%p, rbuf=%p, count=%d, %D, %O, root=%d, %C) failed";
static const char short_spc224[] = "**mpi_reduce_scatter %p %p %p %D %O %C";
static const char long_spc224[] =
    "MPI_Reduce_scatter(sbuf=%p, rbuf=%p, rcnts=%p, %D, %O, %C) failed";
static const char short_spc225[] = "**mpi_register_datarep %s %p %p %p %p";
static const char long_spc225[] =
    "MPI_Register_datarep(datarep=%s, read_conversion_fn=%p, write_conversion_fn=%p, dtype_file_extent_fn=%p, extra_state=%p) failed";
...
static const char long_spc229[] =
    "MPI_Rsend_init(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
...
static const char long_spc231[] =
    "MPI_Scatter(sbuf=%p, scount=%d, %D, rbuf=%p, rcount=%d, %D, root=%d, %C) failed";
static const char short_spc232[] = "**mpi_scatterv %p %p %p %D %p %d %D %d %C";
static const char long_spc232[] =
    "MPI_Scatterv(sbuf=%p, scnts=%p, displs=%p, %D, rbuf=%p, rcount=%d, %D, root=%d, %C) failed";
...
static const char long_spc234[] =
    "MPI_Send_init(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
static const char short_spc235[] = "**mpi_sendrecv %p %d %D %d %d %p %d %D %d %d %C %p";
static const char long_spc235[] =
    "MPI_Sendrecv(sbuf=%p, scount=%d, %D, dest=%d, stag=%d, rbuf=%p, rcount=%d, %D, src=%d, rtag=%d, %C, status=%p) failed";
static const char short_spc236[] = "**mpi_sendrecv_replace %p %d %D %d %d %d %d %C %p";
static const char long_spc236[] =
    "MPI_Sendrecv_replace(buf=%p, count=%d, %D, dest=%d, stag=%d, src=%d, rtag=%d, %C, status=%p) failed";
...
static const char long_spc238[] =
    "MPI_Ssend_init(buf=%p, count=%d, %D, dest=%d, tag=%d, %C, request=%p) failed";
...
static const char long_spc245[] =
    "MPI_Testall(count=%d, req_array=%p, flag=%p, status_array=%p) failed";
static const char short_spc246[] = "**mpi_testany %d %p %p %p %p";
static const char long_spc246[] =
    "MPI_Testany(count=%d, req_array=%p, index=%p, flag=%p, status=%p) failed";
static const char short_spc247[] = "**mpi_testsome %d %p %p %p %p";
static const char long_spc247[] =
    "MPI_Testsome(count=%d, req_array=%p, out_count=%p, indices=%p, status_array=%p) failed";
...
static const char long_spc251[] =
    "MPI_Type_create_darray(size=%d, rank=%d, ndims=%d, array_of_gsizes=%p, array_of_distribs=%p, array_of_dargs=%p, array_of_psizes=%p, order=%d, %D, newtype=%p) failed";
static const char short_spc252[] = "**mpi_type_create_hindexed %d %p %p %D %p";
static const char long_spc252[] =
    "MPI_Type_create_hindexed(count=%d, array_of_blocklengths=%p, array_of_displacements=%p, %D, newtype=%p) failed";
static const char short_spc253[] = "**mpi_type_create_hvector %d %d %d %D %p";
static const char long_spc253[] =
    "MPI_Type_create_hvector(count=%d, blocklength=%d, stride=%d, %D, newtype=%p) failed";
static const char short_spc254[] = "**mpi_type_create_indexed_block %d %d %p %D %p";
static const char long_spc254[] =
    "MPI_Type_create_indexed_block(count=%d, blocklength=%d, array_of_displacements=%p, %D, newtype=%p) failed";
static const char short_spc255[] = "**mpi_type_create_keyval %p %p %p %p";
static const char long_spc255[] =
    "MPI_Type_create_keyval(type_copy_attr_fn=%p, type_delete_attr_fn=%p, type_keyval=%p, extra_state=%p) failed";
static const char short_spc256[] = "**mpi_type_create_resized %D %d %d %p";
static const char long_spc256[] =
    "MPI_Type_create_resized(%D, lb=%d, extent=%d, newtype=%p) failed";
static const char short_spc257[] = "**mpi_type_create_struct %d %p %p %p %p";
static const char long_spc257[] =
    "MPI_Type_create_struct(count=%d, array_of_blocklengths=%p, array_of_displacements=%p, array_of_types=%p, newtype=%p) failed";
static const char short_spc258[] = "**mpi_type_create_subarray %d %p %p %p %d %D %p";
static const char long_spc258[] =
    "MPI_Type_create_subarray(ndims=%d, array_of_sizes=%p, array_of_subsizes=%p, array_of_starts=%p, order=%d, %D, newtype=%p) failed";
...
static const char long_spc264[] =
    "MPI_Type_get_attr(%D, type_keyval=%d, attribute_val=%p, flag=%p) failed";
static const char short_spc265[] = "**mpi_type_get_contents %D %d %d %d %p %p %p";
static const char long_spc265[] =
    "MPI_Type_get_contents(%D, max_integers=%d, max_addresses=%d, max_datatypes=%d, array_of_integers=%p, array_of_addresses=%p, array_of_datatypes=%p) failed";
static const char short_spc266[] = "**mpi_type_get_envelope %D %p %p %p %p";
static const char long_spc266[] =
    "MPI_Type_get_envelope(%D, num_integers=%p, num_addresses=%p, num_datatypes=%p, combiner=%p) failed";
...
static const char long_spc270[] =
    "MPI_Type_hindexed(count=%d, blocklens=%p, indices=%p, %D, new_type_p=%p) failed";
static const char short_spc271[] = "**mpi_type_hvector %d %d %d %D %p";
static const char long_spc271[] =
    "MPI_Type_hvector(count=%d, blocklen=%d, stride=%d, %D, new_type_p=%p) failed";
static const char short_spc272[] = "**mpi_type_indexed %d %p %p %D %p";
static const char long_spc272[] =
    "MPI_Type_indexed(count=%d, blocklens=%p, indices=%p, %D, new_type_p=%p) failed";
...
static const char long_spc278[] =
    "MPI_Type_struct(count=%d, blocklens=%p, indices=%p, old_types=%p, new_type_p=%p) failed";
...
static const char long_spc280[] =
    "MPI_Type_vector(count=%d, blocklength=%d, stride=%d, %D, new_type_p=%p) failed";
static const char short_spc281[] = "**mpi_unpack %p %d %p %p %d %D %C";
static const char long_spc281[] =
    "MPI_Unpack(inbuf=%p, insize=%d, position=%p, outbuf=%p, outcount=%d, %D, %C) failed";
static const char short_spc282[] = "**mpi_unpack_external %s %p %d %p %p %d %D";
static const char long_spc282[] =
    "MPI_Unpack_external(datarep=%s, inbuf=%p, insize=%d, position=%p, outbuf=%p, outcount=%d, %D) failed";
...
static const char long_spc287[] =
    "MPI_Waitsome(count=%d, req_array=%p, out_count=%p, indices=%p, status_array=%p) failed";
...
static const char long_spc290[] =
    "MPI_Win_create(base=%p, size=%d, disp_unit=%d, %I, %C, win=%p) failed";
...
static const char long_spc292[] =
    "MPI_Win_create_keyval(win_copy_attr_fn=%p, win_delete_attr_fn=%p, win_keyval=%p, extra_state=%p) failed";
...
static const char long_spc297[] =
    "MPI_Win_get_attr(%W, win_keyval=%d, attribute_val=%p, flag=%p) failed";
...
static const char long_spc347[] =
    "unable to get the PARENT_ROOT_PORT_NAME from the keyval space (pmi_error %d)";
...
static const char long_spc352[] =
    "The range array specifies duplicate entries; process %d specified in range array %d was previously specified in range array %d";
static const char short_spc353[] = "**rangeendinvalid %d %d %d";
static const char long_spc353[] =
    "The %dth element of a range array ends at %d but must be nonnegative and less than %d";
static const char short_spc354[] = "**rangestartinvalid %d %d %d";
static const char long_spc354[] =
    "The %dth element of a range array starts at %d but must be nonnegative and less than %d";
static const char short_spc355[] = "**rank %d %d";
static const char long_spc355[] =
    "Invalid rank has value %d but must be nonnegative and less than %d";
static const char short_spc356[] = "**rankarray %d %d %d";
static const char long_spc356[] =
    "Invalid rank in rank array at index %d; value is %d but must be in the range 0 to %d";
static const char short_spc357[] = "**rankdup %d %d %d";
static const char long_spc357[] =
    "Duplicate ranks in rank array at index %d, has value %d which is also the value at index %d";
static const char short_spc358[] = "**ranklocal %d %d";
static const char long_spc358[] =
    "Error specifying local_leader; rank given was %d but must be in the range 0 to %d";
static const char short_spc359[] = "**rankremote %d %d";
static const char long_spc359[] =
    "Error specifying remote_leader; rank given was %d but must be in the range 0 to %d";
...
static const char long_spc362[] =
    "Ready send from source %d and with tag %d had no matching receive";
...
static const char long_spc371[] =
    "Shared memory subsystem unable to allocate a block of the requested size, %d bytes";
...
static const char long_spc381[] =
    "operation cancelled because sock was closed locally (set=%d,sock=%d)";
...
static const char long_spc386[] =
    " supplied listener sock is in a bad state (set=%d,sock=%d,state=%d)";
...
static const char long_spc390[] =
    "operating system routine %s failed due to lack of memory (set=%d,sock=%d)";
...
static const char long_spc392[] =
    "the supplied buffer contains invalid memory (set=%d,sock=%d,errno=%d:%s)";
...
static const char long_spc394[] =
    "sock contains an invalid file descriptor (set=%d,sock=%d,fd=%d:%d)";
...
static const char long_spc399[] =
    "fatal error: failed to enqueue an event; event was lost (set=%d,sock=%d,op=%d)";
...
static const char long_spc401[] =
    "unable to resolve host name to an address (set=%d,sock=%d,host=%s)";
...
static const char long_spc407[] =
    "unpected operating system error from %s (set=%d,sock=%d,errno=%d:%s)";
static const char short_spc408[] = "**sock|poll|osnomem %d %d %d %s";
static const char long_spc408[] =
    "operating system routine failed due to lack of memory (set=%d,sock=%d,errno=%d:%s)";
static const char short_spc409[] = "**sock|poll|pipe %d %s";
static const char long_spc409[] =
    "unable to allocate pipe to wakeup a blocking poll() (errno=%d:%s)";
...
static const char long_spc416[] =
    "attempt to perform multiple simultaneous writes (set=%d,sock=%d)";
...
static const char long_spc421[] =
    "Message from rank %d and tag %d truncated; %d bytes received but buffer size is %d";
