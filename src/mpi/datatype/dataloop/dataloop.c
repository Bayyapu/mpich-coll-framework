...
void MPIR_Dataloop_update(DLOOP_Dataloop * dataloop, DLOOP_Offset ptrdiff)
{
    ...
    case DLOOP_KIND_STRUCT:
        ...
        for (i = 0; i < dataloop->loop_params.s_t.count; i++) {
            ...
            DLOOP_Ensure_Offset_fits_in_pointer(DLOOP_VOID_PTR_CAST_TO_OFFSET(char *)looparray[i] +
                                                ptrdiff);
                                  ...
                                  DLOOP_Dataloop ** new_loop_p, DLOOP_Size * new_loop_sz_p)
{
    ...
    case DLOOP_KIND_INDEXED:
         ...
         * - offsets
         */
        new_loop->loop_params.i_t.blocksize_array = (DLOOP_Count *) (((char *) new_loop) + loop_sz);
        ...
        break;
    case DLOOP_KIND_BLOCKINDEXED:
        new_loop->loop_params.bi_t.offset_array = (DLOOP_Offset *) (((char *) new_loop) + loop_sz);
        if (old_loop == NULL) {
...
MPIR_Dataloop_stream_size(struct DLOOP_Dataloop * dl_p, DLOOP_Offset(*sizefn) (DLOOP_Type el_type))
{
    ...
    for (;;) {
        ...
        case DLOOP_KIND_CONTIG:
            ...
            MPL_DBG_MSG_FMT(MPIR_DBG_DATATYPE, VERBOSE,
                            (MPL_DBG_FDEST,
                             "stream_size: contig: ct = %d; new tot_ct = " DLOOP_OFFSET_FMT_DEC_SPEC
