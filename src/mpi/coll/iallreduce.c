                          ...
                          MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if ((count * type_size <= MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE) ||
        ...
        mpi_errno =
            MPIR_Iallreduce_redscat_allgather(sendbuf, recvbuf, count, datatype, op, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
                        ...
                        MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if (nc != NULL) {
        ...
        if ((sendbuf == MPI_IN_PLACE) && (comm_ptr->node_comm->rank != 0)) {
            ...
            mpi_errno =
                nc->coll_fns->Ireduce_sched(sendbuf, recvbuf, count, datatype, op, 0, nc, s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
        MPIR_SCHED_BARRIER(s);
    ...
    if (nrc != NULL) {
        ...
        mpi_errno =
            nrc->coll_fns->Iallreduce_sched(MPI_IN_PLACE, recvbuf, count, datatype, op, nrc, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        MPIR_SCHED_BARRIER(s);
    }
                         ...
                         MPI_Op op, MPIR_Comm * comm_ptr, MPI_Request * request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno =
        comm_ptr->coll_fns->Iallreduce_sched(sendbuf, recvbuf, count, datatype, op, comm_ptr, s);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
