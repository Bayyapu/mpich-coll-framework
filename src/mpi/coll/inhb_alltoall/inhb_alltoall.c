...
cvars:
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALL_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALL_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ineighbor_alltoall will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level ineighbor_alltoall function will not be        called.
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_alltoall_intra_sched
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ineighbor_alltoall_intra_sched(const void *sendbuf, int sendcount,
                                  ...
                                  MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Ineighbor_alltoall_alg_intra_choice) {
            case MPIR_INEIGHBOR_ALLTOALL_ALG_INTRA_GENERIC:
                mpi_errno = MPIR_Ineighbor_alltoall_generic_sched(sendbuf, sendcount, sendtype,
                                               recvbuf, recvcount, recvtype,
                                               comm_ptr, s);
                break;
            case MPIR_INEIGHBOR_ALLTOALL_ALG_INTRA_AUTO:
            ATTRIBUTE((fallthrough));
            default:
                mpi_errno = MPIR_Ineighbor_alltoall_intra_sched(sendbuf, sendcount, sendtype,
                                               recvbuf, recvcount, recvtype,
                                               comm_ptr, s);
                break;
        }
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_INEIGHBOR_ALLTOALL_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
