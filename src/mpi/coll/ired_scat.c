                                  ...
                                  MPIR_Sched_t s)
{
    ...
    for (i = 1; i < comm_size; i++) {
        ...
        if (is_commutative || (src < rank)) {
            if (sendbuf != MPI_IN_PLACE) {
                mpi_errno =
                    MPIR_Sched_reduce(tmp_recvbuf, recvbuf, recvcounts[rank], datatype, op, s);
                if (mpi_errno)
                    MPIR_ERR_POP(mpi_errno);
            } else {
                mpi_errno =
            ...
            if (sendbuf != MPI_IN_PLACE) {
                mpi_errno =
                    MPIR_Sched_reduce(recvbuf, tmp_recvbuf, recvcounts[rank], datatype, op, s);
                if (mpi_errno)
                    MPIR_ERR_POP(mpi_errno);
                MPIR_SCHED_BARRIER(s);
                               ...
                               MPIR_Sched_t s)
{
    ...
    if (is_commutative && (nbytes < MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        mpi_errno =
            MPIR_Ireduce_scatter_rec_hlv(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else if (is_commutative && (nbytes >= MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        mpi_errno =
            MPIR_Ireduce_scatter_pairwise(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {    /* (!is_commutative) */
        ...
        if (MPIU_is_pof2(comm_size, NULL) && is_block_regular) {
            ...
            mpi_errno =
                MPIR_Ireduce_scatter_noncomm(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr,
                                             s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        } else {
            ...
            mpi_errno =
                MPIR_Ireduce_scatter_rec_dbl(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr,
                                             s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
                              ...
                              MPI_Request * request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno =
        comm_ptr->coll_fns->Ireduce_scatter_sched(sendbuf, recvbuf, recvcounts, datatype, op,
                                                  comm_ptr, s);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
