                       ...
                       MPI_Op op, int root, MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if ((count * type_size > MPIR_CVAR_REDUCE_SHORT_MSG_SIZE) &&
        ...
        mpi_errno =
            MPIR_Ireduce_redscat_gather(sendbuf, recvbuf, count, datatype, op, root, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
...
*/
    ...
    if (root == MPI_ROOT) {
        ...
        mpi_errno =
            MPIR_Ireduce_intra(sendbuf, tmp_buf, count, datatype, op, 0, comm_ptr->local_comm, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        mpi_errno = MPIR_Sched_barrier(s);
        if (mpi_errno)
                      ...
                      MPI_Op op, int root, MPIR_Comm * comm_ptr, MPI_Request * request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno =
        comm_ptr->coll_fns->Ireduce_sched(sendbuf, recvbuf, count, datatype, op, root, comm_ptr, s);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
