...
cvars:
    - name        : MPIR_CVAR_NEIGHBOR_ALLGATHERV_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        ...
        nonblocking - Force nonblocking algorithm
    ...
    - name        : MPIR_CVAR_NEIGHBOR_ALLGATHERV_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Neighbor_allgatherv will use allow the device to override the default, MPIR-level collective algorithms. The device still has the
...
int MPIR_Neighbor_allgatherv_intra(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                      int recvcounts[], const int displs[], MPI_Datatype recvtype, MPI_Comm comm)
{
...
int MPIR_Neighbor_allgatherv(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, const int recvcounts[], const int displs[], MPI_Datatype recvtype, MPIR_Comm *comm_ptr)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Neighbor_allgatherv_alg_intra_choice) {
            case MPIR_NEIGHBOR_ALLGATHERV_ALG_INTRA_NONBLOCKING:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm_ptr);
                break;
            case MPIR_NEIGHBOR_ALLGATHERV_ALG_INTRA_AUTO:
            ATTRIBUTE((fallthrough));
            default:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendty    pe, recvbuf, recvcounts, displs, recvtype, comm_ptr);
                break;
        }
    } else {
        /* intercommunicator */
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_NEIGHBOR_ALLGATHERV_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
