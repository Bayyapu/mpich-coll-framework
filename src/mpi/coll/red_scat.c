...
*/
    ...
    if (!is_commutative) {
        ...
        if (pof2 == comm_size && is_block_regular) {
            ...
            mpi_errno =
                MPIR_Reduce_scatter_noncomm(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr,
                                            errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                       ...
                       MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
    ...
    mpi_errno = MPID_Reduce_scatter(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, &errflag);
    if (mpi_errno)
        goto fn_fail;
