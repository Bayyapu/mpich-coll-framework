...
cvars:
    - name        : MPIR_CVAR_NEIGHBOR_ALLGATHER_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        Variable to select neighbor_allgather algorithm
    ...
    - name        : MPIR_CVAR_NEIGHBOR_ALLGATHER_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Neighbor_allgather will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level neighbor_allgather function will not be        called.
...
int MPIR_Neighbor_allgather_intra(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
...
int MPIR_Neighbor_allgather(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPIR_Comm *comm_ptr)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Neighbor_allgather_alg_intra_choice) {
            case MPIR_NEIGHBOR_ALLGATHER_ALG_INTRA_NONBLOCKING:
                mpi_errno = MPIR_Neighbor_allgather_nb(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr);
                break;
            case MPIR_NEIGHBOR_ALLGATHER_ALG_INTRA_AUTO:
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_NEIGHBOR_ALLGATHER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
