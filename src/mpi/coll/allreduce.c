...
static inline int allreduce_intra_or_coll_fn(void *sendbuf, void *recvbuf, int count,
                                             ...
                                             MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->coll_fns != NULL && comm_ptr->coll_fns->Allreduce != NULL) {
        /* --BEGIN USEREXTENSION-- */
        mpi_errno =
            comm_ptr->coll_fns->Allreduce(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
        if (mpi_errno)
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allreduce_intra(const void *sendbuf,
                         void *recvbuf,
                         int count,
                         ...
                         MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (MPIR_CVAR_ENABLE_SMP_COLLECTIVES && MPIR_CVAR_ENABLE_SMP_ALLREDUCE) {
        ...
        if (MPIR_Comm_is_node_aware(comm_ptr) && is_commutative &&
            ...
            if (comm_ptr->node_roots_comm != NULL) {
                mpi_errno =
                    allreduce_intra_or_coll_fn(MPI_IN_PLACE, recvbuf, count, datatype, op,
    ...
    if (!is_homogeneous) {
        ...
        mpi_errno = MPID_Reduce(sendbuf, recvbuf, count, datatype, op, 0, comm_ptr, errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
...
#endif /* MPID_HAS_HETERO */
    {
        ...
        if ((nbytes <= MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE) ||
            ...
            mpi_errno =
                MPIR_Allreduce_recursive_doubling(sendbuf, recvbuf, count, datatype, op, comm_ptr,
                                                  errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            ...
            mpi_errno =
                MPIR_Allreduce_reduce_scatter_allgather(sendbuf, recvbuf, count, datatype, op,
                                                        comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allreduce_inter(const void *sendbuf,
                         void *recvbuf,
                         int count,
                         MPI_Datatype datatype,
                         MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allreduce_recursive_doubling(const void *sendbuf,
                                      void *recvbuf,
                                      int count,
                                      MPI_Datatype datatype,
                                      MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allreduce_reduce_scatter_allgather(const void *sendbuf,
                                            void *recvbuf,
                                            int count,
                                            MPI_Datatype datatype,
                                            MPI_Op op,
                        ...
                        MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->coll_fns != NULL && comm_ptr->coll_fns->Allreduce != NULL) {
        ...
        if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
            /* intracommunicator */
            mpi_errno =
                MPIR_Allreduce_intra(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        } else {
            /* intercommunicator */
            mpi_errno =
                MPIR_Allreduce_inter(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
    }
