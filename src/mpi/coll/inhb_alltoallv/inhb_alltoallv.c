...
cvars:
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALLV_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALLV_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ineighbor_alltoallv will use allow the device to override the
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_alltoallv_intra_sched
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ineighbor_alltoallv_intra_sched(const void *sendbuf, const int sendcounts[],
                                   ...
                                   MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Ineighbor_alltoallv_alg_intra_choice) {
            case MPIR_INEIGHBOR_ALLTOALLV_ALG_INTRA_GENERIC:
                mpi_errno = MPIR_Ineighbor_alltoallv_generic_sched(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm_ptr, request);
                break;
            case MPIR_INEIGHBOR_ALLTOALLV_ALG_INTRA_AUTO:
            ATTRIBUTE((fallthrough));
            default:
                mpi_errno = MPIR_Ineighbor_alltoallv_intra_sched(sendbuf, sendcounts, sdispls    , sendtype, recvbuf, recvcounts, rdispls, recvtype, comm_ptr, request);
                break;
        }
    } else {
        /* intercommunicator */
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_INEIGHBOR_ALLTOALLV_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_CO    LLECTIVES) {
        ...
        mpi_errno = MPIR_Ineighbor_alltoallv(sendbuf, sendcounts, sdispls, sendtype, re    cvbuf, recvcounts, rdispls, recvtype, comm_ptr, request);
    }
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
