    ...
    for (i = 1; i < comm_size; i++) {
        ...
        if (is_commutative || (src < rank)) {
            if (sendbuf != MPI_IN_PLACE) {
                ...
                mpi_errno =
                    MPIR_Sched_reduce(tmp_recvbuf, ((char *) recvbuf + disps[rank] * extent),
    ...
    for (i = 0; i < comm_size; ++i) {
        mpi_errno =
            MPIR_Sched_copy(((char *) (sendbuf ==
                            ...
                            ((char *) tmp_buf0 +
                             (MPIU_Mirror_permutation(i, log2_comm_size) * true_extent *
                                     ...
                                     MPIR_Sched_t s)
{
    ...
    if (is_commutative && (nbytes < MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        mpi_errno =
            MPIR_Ireduce_scatter_block_rec_hlv(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr,
                                               s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else if (is_commutative && (nbytes >= MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        mpi_errno =
            MPIR_Ireduce_scatter_block_pairwise(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr,
                                                s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {    /* (!is_commutative) */
        ...
        if (MPIU_is_pof2(comm_size, NULL)) {
            ...
            mpi_errno =
                MPIR_Ireduce_scatter_block_noncomm(sendbuf, recvbuf, recvcount, datatype, op,
                                                   comm_ptr, s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        } else {
            ...
            mpi_errno =
                MPIR_Ireduce_scatter_block_rec_dbl(sendbuf, recvbuf, recvcount, datatype, op,
                                                   comm_ptr, s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
                                    ...
                                    MPI_Request * request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno =
        comm_ptr->coll_fns->Ireduce_scatter_block_sched(sendbuf, recvbuf, recvcount, datatype, op,
                                                        comm_ptr, s);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
                              ...
                              MPI_Request * request)
{
    ...
    mpi_errno =
        MPID_Ireduce_scatter_block(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr, request);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
