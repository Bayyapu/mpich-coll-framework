...
cvars:
    ...
    - name        : MPIR_CVAR_IREDUCE_SCATTER_BLOCK_ALGORITHM_INTER
      ...
      description : >-
        Variable to select ireduce_scatter_block algorithm
        auto - Internal algorithm selection
        default_inter - Force default inter algorithm
                                     ...
                                     MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        ...
        switch (MPIR_Ireduce_scatter_block_alg_intra_choice) {
            ...
            case MPIR_IREDUCE_SCATTER_BLOCK_ALG_INTRA_PAIRWISE:
                mpi_errno = MPIR_Ireduce_scatter_block_pairwise_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
                break;
            case MPIR_IREDUCE_SCATTER_BLOCK_ALG_INTRA_REC_HLV:
       ...
       switch (MPIR_Ireduce_scatter_block_alg_inter_choice) {
           case MPIR_IREDUCE_SCATTER_BLOCK_ALG_DEFAULT_INTER:
               mpi_errno = MPIR_Ireduce_scatter_block_default_inter_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
               break;
