...
cvars:
    ...
    - name        : MPIR_CVAR_IALLGATHERV_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Iallgatherv will use allow the device to override the        default, MPIR-level collective algorithms. The device still has the
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        ...
        switch (MPIR_Iallgatherv_alg_intra_choice) {
            case MPIR_IALLGATHERV_ALG_INTRA_BRUCKS:
                mpi_errno = MPIR_Iallgather_brucks_sched(sendbuf, sendcount, sendtype,
