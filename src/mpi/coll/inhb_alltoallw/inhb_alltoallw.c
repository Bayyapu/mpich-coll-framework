...
cvars:
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALLW_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_INEIGHBOR_ALLTOALLW_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ineighbor_alltoallw will use allow the device to override the
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_alltoallw_intra_sched
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ineighbor_alltoallw_intra_sched(const void *sendbuf, const int sendcounts[],
                                   const MPI_Aint sdispls[], const MPI_Datatype sendtypes[],
                                   ...
                                   MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Ineighbor_alltoallw_alg_intra_choice) {
            case MPIR_INEIGHBOR_ALLTOALLW_ALG_INTRA_GENERIC:
                mpi_errno = MPID_Ineighbor_alltoallw_generic_sched(sendbuf, sendcounts, sdispls, sendtypes,
                                               recvbuf, recvcounts, rdispls, recvtypes,
                                               comm_ptr, s);
                break;
            case MPIR_INEIGHBOR_ALLTOALLW_ALG_INTRA_AUTO:
            ATTRIBUTE((fallthrough));
            default:
                mpi_errno = MPID_Ineighbor_alltoallw_intra_sched(sendbuf, sendcounts, sdispls, sendtypes,
                                               recvbuf, recvcounts, rdispls, recvtypes,
                                               comm_ptr, s);
                break;
        }
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_INEIGHBOR_ALLTOALLW_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
