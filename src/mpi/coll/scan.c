...
static int MPIR_Scan_generic(const void *sendbuf,
                             void *recvbuf,
                             int count,
                             ...
                             MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    while (mask < comm_size) {
        ...
        if (dst < comm_size) {
            ...
            if (rank > dst) {
                mpi_errno = MPIR_Reduce_local_impl(tmp_buf, partial_scan, count, datatype, op);
                if (mpi_errno)
                    MPIR_ERR_POP(mpi_errno);
                ...
                if (is_commutative) {
                    ...
                    mpi_errno = MPIR_Reduce_local_impl(partial_scan, tmp_buf, count, datatype, op);
                    if (mpi_errno)
                        MPIR_ERR_POP(mpi_errno);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Scan(const void *sendbuf,
              void *recvbuf,
              int count,
              MPI_Datatype datatype, MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->node_comm != NULL) {
        mpi_errno = MPID_Scan(sendbuf, recvbuf, count, datatype, op, comm_ptr->node_comm, errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
    ...
    if (noneed == 0) {
        if (comm_ptr->node_comm != NULL) {
            mpi_errno = MPID_Bcast(tempbuf, count, datatype, 0, comm_ptr->node_comm, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
        ...
        mpi_errno = MPIR_Reduce_local_impl(tempbuf, recvbuf, count, datatype, op);
    }
