                         ...
                         int recvcount, MPI_Datatype recvtype, MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if (sendbuf == MPI_IN_PLACE) {
        mpi_errno =
            MPIR_Ialltoall_inplace(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                   comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
         ...
         * IEEE TPDS, Nov. 97 */
        mpi_errno =
            MPIR_Ialltoall_bruck(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                 comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        ...
        mpi_errno =
            MPIR_Ialltoall_perm_sr(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                   comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        ...
        mpi_errno =
            MPIR_Ialltoall_pairwise(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                    comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
                        ...
                        MPI_Request * request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno =
        comm_ptr->coll_fns->Ialltoall_sched(sendbuf, sendcount, sendtype, recvbuf, recvcount,
                                            recvtype, comm_ptr, s);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
