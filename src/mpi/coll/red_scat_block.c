...
static int MPIR_Reduce_scatter_block_noncomm(const void *sendbuf,
                                             void *recvbuf,
                                             int recvcount,
...
int MPIR_Reduce_scatter_block_intra(const void *sendbuf,
                                    void *recvbuf,
                                    int recvcount,
                                    MPI_Datatype datatype,
...
*/
    ...
    if (!is_commutative) {
        ...
        if (!(comm_size & (comm_size - 1))) {
            ...
            mpi_errno =
                MPIR_Reduce_scatter_block_noncomm(sendbuf, recvbuf, recvcount, datatype, op,
                                                  comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
/* not declared static because a machine-specific function may call this one in some cases */
int MPIR_Reduce_scatter_block_inter(const void *sendbuf,
                                    void *recvbuf,
                                    int recvcount,
                                    MPI_Datatype datatype,
                                    MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
                              ...
                              MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno =
            MPIR_Reduce_scatter_block_intra(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr,
                                            errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno =
            MPIR_Reduce_scatter_block_inter(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr,
                                            errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
                                   ...
                                   MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->coll_fns != NULL && comm_ptr->coll_fns->Reduce_scatter_block != NULL) {
        /* --BEGIN USEREXTENSION-- */
        mpi_errno =
            comm_ptr->coll_fns->Reduce_scatter_block(sendbuf, recvbuf, recvcount, datatype, op,
                                                     comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        /* --END USEREXTENSION-- */
