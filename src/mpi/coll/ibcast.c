                                       ...
                                       MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    for (i = 1; i < comm_size; i++) {
        ...
        mpi_errno = MPIR_Sched_recv_status(((char *) tmp_buf + left_disp),
                                           left_count, MPI_BYTE, left, comm_ptr, &status->status,
                                           s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        MPIR_SCHED_BARRIER(s);
                    ...
                    MPIR_Sched_t s)
{
    ...
    if (comm_ptr->node_comm != NULL) {
        mpi_errno =
            comm_ptr->node_comm->coll_fns->Ibcast_sched(buffer, count, datatype, 0,
                      ...
                      MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) || (comm_size < MPIR_CVAR_BCAST_MIN_PROCS)) {
        ...
        if ((nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE) && (MPIU_is_pof2(comm_size, NULL))) {
            mpi_errno =
                MPIR_Ibcast_scatter_rec_dbl_allgather(buffer, count, datatype, root, comm_ptr, s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        } else {
            mpi_errno =
                MPIR_Ibcast_scatter_ring_allgather(buffer, count, datatype, root, comm_ptr, s);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
    }
                      ...
                      MPIR_Comm * comm_ptr, MPIR_Sched_t s)
{
    ...
    if (root == MPI_PROC_NULL) {
        ...
        mpi_errno =
            comm_ptr->local_comm->coll_fns->Ibcast_sched(buffer, count, datatype, root,
