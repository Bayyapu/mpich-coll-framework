...
MPI_ERR_FILE
    ...
    MPI_ERR_TYPE MPIR_ERR_TYPE_NULL(null datatype.from MPICH)
    ...
    MPI_ERR_REQUEST MPIR_ERR_REQUEST_NULL(null request.from MPICH)
MPI_ERR_IO
    MPIR_ERR_ETYPE_FRACTIONAL "Only an integral number of etypes can be accessed"
    MPIR_ERR_NO_FSTYPE
    "Can't determine the file-system type. Check the filename/path you provided and try again. Otherwise, prefix the filename with a string to indicate the type of file sytem (piofs:, pfs:, nfs:, ufs:, hfs:, xfs:, sfs:, pvfs:, panfs: ftp: gsiftp:)"
    MPIR_ERR_NO_UFS "ROMIO has not been configured to use the UFS file system" MPIR_ERR_NO_NFS
    "ROMIO has not been configured to use the NFS file system" MPIR_ERR_NO_XFS
    "ROMIO has not been configured to use the XFS file system" MPIR_ERR_NO_PVFS2
    "ROMIO has not been configured to use the PVFS2 file system" MPIR_ERR_NO_PANFS
    "ROMIO has not been configured to use the PANFS file system" MPIR_ERR_MULTIPLE_SPLIT_COLL
    "Only one active split collective I/O operation allowed per file handle"
    MPIR_ERR_NO_SPLIT_COLL "No previous split collective begin" MPIR_ERR_ASYNC_OUTSTANDING
    "There are outstanding nonblocking I/O operations on this file" MPIR_ADIO_ERROR "I/O Error"
strerror(errno) MPIR_READ_PERM
"ROMIO tries to optimize this access by doing a read-modify-write, but is unable to read the file. Please give the file read permission and open it with MPI_MODE_RDWR." MPIR_PREALLOC_PERM
...
MPIR_ERR_FILETYPE "Filetype must be constructed out of one or more etypes" MPIR_ERR_NO_TESTFS "ROMIO has not been configured to use the TESTFS file system" MPIR_ERR_DEFERRED
"independent IO attempted even though no_indep_rw hint given" MPIR_ERR_NO_GPFS "ROMIO has not been configured to use the GPFS file system" MPIR_ERR_NO_IME
"ROMIO has not been configured to use the IME burst buffer" MPI_ERR_COMM MPIR_ERR_COMM_NULL(null communicator.
from MPICH) MPIR_ERR_COMM_INTER(no intercommunicator.(from MPICH) MPI_ERR_UNSUPPORTED_DATAREP
