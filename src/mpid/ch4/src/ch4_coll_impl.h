                                                           ...
                                                           ch4_algo_parameters_container)
{
    ...
    if (comm->is_single_node) {
        mpi_errno =
            MPIDI_SHM_mpi_bcast(buffer, count, datatype, root, comm, errflag, bcast_container);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        mpi_errno =
            MPIDI_NM_mpi_bcast(buffer, count, datatype, root, comm, errflag, bcast_container);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
#endif /*MPIDI_BUILD_CH4_SHM */
                                                    ...
                                                    ATTRIBUTE((unused)))
{
    ...
    mpi_errno = MPIR_Reduce_inter(sendbuf, recvbuf, count, datatype, op, root, comm, errflag);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
